...
const books=new require('./books_dumb');
...
router.get('/books/:id',function(req,res){
    ...
    if(!book) return res.status(404).send('Book does not exit');
...
router.post('/books',function(req,res){
    ...
    const bookExit=booksDirectory.find(b=>b.isbn===isbn);
    if(bookExit) return res.send("Book alredy exits");
    const book={
        title,
        isbn,
        pageCount,
